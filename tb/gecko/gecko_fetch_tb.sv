...
#(
    ...
    initial begin
        ...
        fork
            ...
            begin
                jump_command.send('{update_pc: 'b0, branched: 'b1, jumped: 'b0, current_pc: 'd0,
                                  actual_next_pc: 'd4, halt: 'b0,
                                  prediction: '{miss: 'b1, history: GECKO_BRANCH_PREDICTOR_HISTORY_STRONG_TAKEN}});
                @(posedge clk);
                @(posedge clk);
                jump_command.send(
                    '{update_pc: 'b1, branched: 'b1, jumped: 'b0, current_pc: 'd8, actual_next_pc: 'd4, halt: 'b0,
                    prediction: '{miss: 'b0, history: GECKO_BRANCH_PREDICTOR_HISTORY_STRONG_NOT_TAKEN}});
                @(posedge clk);
                @(posedge clk);
                jump_command.send('{update_pc: 'b1, branched: 'b1, jumped: 'b0, current_pc: 'b0,
                                  actual_next_pc: 'd4, halt: 'b1,
                                  prediction: '{miss: 'b0, history: GECKO_BRANCH_PREDICTOR_HISTORY_STRONG_TAKEN}});
            end
        join
    end
