...
#(
    ...
    function automatic shortreal bitstoshortreal(input logic [31:0] bits);
        logic sign;
        logic [7:0] exp;
        logic [22:0] frac;
        shortreal sr;
    ...
    function automatic logic [31:0] shortrealtobits(input shortreal r);
        logic sign;
        integer iexp;
        logic [7:0] exp;
        logic [22:0] frac;
    ...
    function automatic shortreal result_to_float(input basilisk_result_t result);
        fpu_float_fields_t f = fpu_operations_round(result.result);
        return bitstoshortreal({f.sign, f.exponent, f.mantissa});
    endfunction

    function automatic fpu_result_t bits_to_result(input rv32_reg_value_t value);
        return
        '{
            sign: value[31],
    ...
    std_stream_intf #(
        .T(basilisk_result_t)
    ) partial_memory_result (
        .clk,
        .rst
    );
    std_stream_intf #(
        .T(basilisk_result_t)
    ) pipelined_memory_result (
        .clk,
        .rst
    );
    ...
    initial begin
        ...
        fork
            begin
                sqrt_command.send('{dest_reg_addr: 'h14, dest_offset_addr: 'h14, a: input_fields[2],
                                  conditions_a: input_conditions[2], mode: FPU_ROUND_MODE_EVEN});
            end
            begin
                divide_command.send('{dest_reg_addr: 'h15, dest_offset_addr: 'h15, a: input_fields[3],
                                    conditions_a: input_conditions[3], b: input_fields[2],
                                    conditions_b: input_conditions[2], mode: FPU_ROUND_MODE_EVEN});
            end
            begin
                @(posedge clk);
                @(posedge clk);
                ...
                @(posedge clk);
                @(posedge clk);
                add_command.send('{dest_reg_addr: 'h8, dest_offset_addr: 'h8, a: input_fields[4],
                                 conditions_a: input_conditions[4], b: input_fields[4],
                                 conditions_b: input_conditions[4], mode: FPU_ROUND_MODE_EVEN});
            end
            begin
                mult_command.send('{dest_reg_addr: 'h16, dest_offset_addr: 'h16, enable_macc: 'b0,
                                  a: input_fields[4], conditions_a: input_conditions[4], b: input_fields[4],
                                  conditions_b: input_conditions[4], c: input_fields[1],
                                  conditions_c: input_conditions[1], mode: FPU_ROUND_MODE_EVEN});
                mult_command.send('{dest_reg_addr: 'h17, dest_offset_addr: 'h17, enable_macc: 'b1,
                                  a: input_fields[4], conditions_a: input_conditions[4], b: input_fields[4],
                                  conditions_b: input_conditions[4], c: input_fields[1],
                                  conditions_c: input_conditions[1], mode: FPU_ROUND_MODE_EVEN});
            end
            begin
                memory_command.send('{dest_reg_addr: 'b0, dest_offset_addr: 'b0, a: 'h7f80_0000,
                                    op: BASILISK_MEMORY_OP_STORE, mem_base_addr: 'h0, mem_offset_addr: 'h0});
                memory_command.send('{dest_reg_addr: 'b0, dest_offset_addr: 'b1, a: 'hffff_0000,
                                    op: BASILISK_MEMORY_OP_LOAD, mem_base_addr: 'h0, mem_offset_addr: 'h0});
            end
            begin
                convert_command.send('{dest_reg_addr: 'b1, dest_offset_addr: 'b1, a: input_fields[1],
                                     conditions_a: input_conditions[1], b: input_fields[2],
                                     conditions_b: input_conditions[2], op: BASILISK_CONVERT_OP_MAX,
                                     signed_integer: 'b0});
                convert_command.send('{dest_reg_addr: 'b1, dest_offset_addr: 'h2, a: input_fields[1],
                                     conditions_a: input_conditions[1], b: input_fields[2],
                                     conditions_b: input_conditions[2], op: BASILISK_CONVERT_OP_MIN,
                                     signed_integer: 'b0});
                convert_command.send('{dest_reg_addr: 'b1, dest_offset_addr: 'h3, a: 'd23, conditions_a: 'b0,
                                     b: input_fields[2], conditions_b: input_conditions[2],
                                     op: BASILISK_CONVERT_OP_CNV, signed_integer: 'b0});
                convert_command.send('{dest_reg_addr: 'b1, dest_offset_addr: 'h3, a: 'd1000000001,
                                     conditions_a: 'b0, b: input_fields[2], conditions_b: input_conditions[2],
                                     op: BASILISK_CONVERT_OP_CNV, signed_integer: 'b0});
            end
            begin
            ...
            begin
                while ('b1) begin
                    ...
                    $display("Result Dest: %h, Offset: %h, Value: %f, Raw: %h", writeback_result_value.dest_reg_addr,
                             writeback_result_value.dest_offset_addr, bitstoshortreal(writeback_result_value.result),
