...
package fpu_reference_pkg;
    ...
    function automatic fpu_float_fields_t fpu_reference_float_add(input fpu_float_fields_t a, b,
        ...
        if (conditions_A.infinity && !conditions_B.infinity) y = a;
        else if (conditions_B.infinity && !conditions_A.infinity) y = b;
        else if (conditions_A.nan || conditions_B.nan) y = FPU_FLOAT_NAN;
        else if (conditions_A.infinity && conditions_B.infinity && (a.sign == b.sign)) y = a;
        else if (conditions_A.infinity && conditions_B.infinity && (a.sign != b.sign)) y = FPU_FLOAT_NAN;
    ...
    function automatic fpu_float_fields_t fpu_reference_float_mult(input fpu_float_fields_t a, b,
                                                                   input fpu_round_mode_t mode);
        ...
        end else if (leading_mzs < 47 && ~expY_ex[8] && expY_ex <= leading_mzs) begin
            sig_product = sig_product << expY_ex;
            expY_ex = 0;
        end else if (leading_mzs < 47 && expY_ex[8]) begin
        ...
        if (a.exponent == 8'hFF || b.exponent == 8'hFF) begin
            ...
            else if (conditions_A.infinity && conditions_B.zero)  // infinity times zero
                {y.sign, y.exponent, y.mantissa} = FPU_FLOAT_NAN;
            else if (conditions_B.infinity && conditions_A.zero) {y.sign, y.exponent, y.mantissa} = FPU_FLOAT_NAN;
            else if (conditions_A.infinity && !(conditions_B.infinity))  // infinity times a number
                {y.exponent, y.mantissa} = {a.exponent, a.mantissa};
            else if (conditions_B.infinity && !(conditions_A.infinity))
                {y.exponent, y.mantissa} = {b.exponent, b.mantissa};
            else if (conditions_A.infinity && conditions_B.infinity)  // infinity times infinity
                {y.exponent, y.mantissa} = {a.exponent, a.mantissa};
            else {y.sign, y.exponent, y.mantissa} = FPU_FLOAT_NAN;
        end else if (overflow) y = (sign) ? FPU_FLOAT_NEG_INF : FPU_FLOAT_POS_INF;
        else if (underflow) y = FPU_FLOAT_ZERO;


        return y;
    endfunction

    function automatic fpu_float_fields_t fpu_reference_float_div(input fpu_float_fields_t a, b,
                                                                  input fpu_round_mode_t mode);
        ...
        if (conditions_A.nan || conditions_B.nan) y = FPU_FLOAT_NAN;
        else if (conditions_B.zero || conditions_B.infinity) y = FPU_FLOAT_NAN;
        else if (overflow || conditions_A.infinity) y = (y.sign) ? FPU_FLOAT_NEG_INF : FPU_FLOAT_POS_INF;
        else if (underflow || conditions_A.zero) y = FPU_FLOAT_ZERO;
        return y;
    endfunction
    ...
    function automatic fpu_float_fields_t fpu_reference_float_sqrt(input fpu_float_fields_t number,
                                                                   input fpu_round_mode_t mode);
