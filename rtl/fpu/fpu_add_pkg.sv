...
package fpu_add_pkg;
    ...
    function automatic fpu_add_exp_result_t fpu_float_add_exponent(
        ...
        result.infinity = conditions_A.infinity || conditions_B.infinity;
    ...
    function automatic fpu_add_op_result_t fpu_float_add_operation(input fpu_add_exp_result_t exp_result);
        ...
        result.carry = 0;
        case ({
            exp_result.sign_A, exp_result.sign_B
        })
            2'b00: begin
    ...
    function automatic fpu_result_t fpu_float_add_normalize(input fpu_add_op_result_t y);
        ...
        if ({y.carry, sum} == 0) result.exponent = 0;
        else if (y.carry) begin
            sticky = fpu_get_sticky_bit_27(sum, 5'd1);
            sum = {y.carry, sum[26:1]};
            sum[0] = sum[0] | sticky;
            result.exponent += 1;
            if (result.exponent == 8'd255) overflow = 1;
        end else if (!sum[26]) begin
            leading_zeros = fpu_get_leading_zeros_27(sum);
            sum = sum << leading_zeros;
            result.exponent -= leading_zeros;
        end

        result.sign = y.sign;
        result.mantissa = sum[26:3];
        result.guard = sum[2:0];
        result.nan = y.nan;
