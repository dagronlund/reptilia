...
#(
    ...
    stream_controller #(
        ...
        .valid_output({
            next_mem_command.valid, next_mem_request.valid, next_execute_result.valid, next_jump_command.valid
        }),
        .ready_output({
            next_mem_command.ready, next_mem_request.ready, next_execute_result.ready, next_jump_command.ready
        }),
    ...
    always_comb begin
        ...
        case (cmd_in.op_type)
            GECKO_EXECUTE_TYPE_EXECUTE: begin
                ...
                if (cmd_in.op.ir == RISCV32I_FUNCT3_IR_ADD_SUB || cmd_in.op.ir == RISCV32I_FUNCT3_IR_SRL_SRA) begin
        ...
        if ((current_iteration != 'b0) && ENABLE_INTEGER_MATH) begin  // Math loop iterations
            ...
            case (current_math_op.math_op)
                ...
                next_execute_result.payload.value = next_math_op.operand_value;
    ...
    always_comb instruction_executed_next = (!mispredicted || execute_command.payload.pc_updated) && consume && enable;
