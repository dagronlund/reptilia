...
fn write_tty(tty_out: &StreamPort) -> Result<(), TestError> {
    ...
    let byte = tty_out.payload.get_u64()? as u8;
...
async fn gecko_core(ctx: RustdvCtx) -> Result<(), TestError> {
    ...
    let tty_in = StreamPort::new(&dut, "tty_in")?;
    let tty_out = StreamPort::new(&dut, "tty_out")?;
    tty_in.idle_input();
    tty_out.ready.set_u64(1);

    let exit_flag = dut.signal("exit_flag")?;
    let error_flag = dut.signal("error_flag")?;
    let exit_code = dut.signal("exit_code")?;
    ...
    for _ in 0..MAX_CYCLES {
        ...
        if exit_flag.is_high() {
            let code = exit_code.get_u64()? as u8;
            if code != 0 {
                return Err(TestError::new(format!(
