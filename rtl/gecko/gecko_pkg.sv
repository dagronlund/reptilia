...
package gecko_pkg;
    ...
    function automatic logic gecko_predictor_is_taken(input gecko_predictor_history_t history);
        return history == GECKO_PREDICTOR_HISTORY_STRONG_TAKEN || history == GECKO_PREDICTOR_HISTORY_TAKEN;
    endfunction

    function automatic gecko_predictor_history_t gecko_predictor_update_history(input gecko_predictor_history_t history,
                                                                                input logic took_branch);
        unique case (history)
            GECKO_PREDICTOR_HISTORY_STRONG_TAKEN:
            return took_branch ? GECKO_PREDICTOR_HISTORY_STRONG_TAKEN : GECKO_PREDICTOR_HISTORY_TAKEN;
            GECKO_PREDICTOR_HISTORY_TAKEN:
            return took_branch ? GECKO_PREDICTOR_HISTORY_STRONG_TAKEN : GECKO_PREDICTOR_HISTORY_STRONG_NOT_TAKEN;
            GECKO_PREDICTOR_HISTORY_NOT_TAKEN:
            return took_branch ? GECKO_PREDICTOR_HISTORY_STRONG_TAKEN : GECKO_PREDICTOR_HISTORY_STRONG_NOT_TAKEN;
            GECKO_PREDICTOR_HISTORY_STRONG_NOT_TAKEN:
            return took_branch ? GECKO_PREDICTOR_HISTORY_NOT_TAKEN : GECKO_PREDICTOR_HISTORY_STRONG_NOT_TAKEN;
        endcase
    endfunction
    ...
    function automatic gecko_forwarded_t gecko_construct_forward(input logic valid, input gecko_operation_t op);
        return
        '{
            addr: op.addr,
    ...
    function automatic gecko_add_sub_result_t gecko_add_sub(input riscv32_reg_value_t a, input riscv32_reg_value_t b,
                                                            input logic sub);
    ...
    function automatic gecko_math_result_t gecko_get_full_math_result(input riscv32_reg_value_t a, b, input logic alt);
        logic sub_overflow;
        ...
        riscv32_reg_value_t add_sub_sum = add_sub_result.sum;

        sub_overflow = (a[31] != b[31]) && (a[31] != add_sub_sum[31]);
        ...
        math_result.lt = add_sub_sum[31] ^ sub_overflow;
    ...
    function automatic logic gecko_evaluate_branch(input gecko_math_result_t result, input riscv32i_funct3_t op);
        case (op.b)
    ...
    function automatic gecko_store_result_t gecko_get_store_result(
        input riscv32_reg_value_t value, input gecko_byte_offset_t byte_offset, input riscv32i_funct3_ls_t mem_op);
        unique case (mem_op)
            RISCV32I_FUNCT3_LS_B, RISCV32I_FUNCT3_LS_BU: begin
                return '{value: {value[7:0], value[7:0], value[7:0], value[7:0]}, mask: 4'b1 << byte_offset};
            end
            RISCV32I_FUNCT3_LS_H, RISCV32I_FUNCT3_LS_HU: begin
                return '{value: {value[15:0], value[15:0]}, mask: 4'b11 << {byte_offset[1], 1'b0}};
            end
            default: begin  // RISCV32I_FUNCT3_LS_W
                return '{value: value, mask: 4'b1111};
            end
        endcase
    endfunction
    ...
    function automatic gecko_operation_t gecko_get_load_operation(input gecko_mem_operation_t mem_op,
    ...
    function automatic gecko_math_operation_t gecko_math_operation_step(input gecko_math_operation_t op_input,
...
'{
    ...
    function automatic gecko_config_t gecko_get_basic_config(
        input int instruction_memory_latency, input int data_memory_latency, input int float_memory_latency);
