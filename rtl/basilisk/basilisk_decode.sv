...
#(
    ...
    function automatic fpu_round_mode_t basilisk_decode_find_rounding_mode(rv32f_funct3_round_t inst_round_mode,
    ...
    function automatic basilisk_vector_status_reg_t basilisk_decode_update_reg_status(
        input basilisk_vector_status_reg_t current_status, input basilisk_vector_status_count_t current_status_count,
        input logic vector_writeback_enables[BASILISK_COMPUTE_WIDTH],
        input rv32_reg_addr_t vector_writeback_addresses[BASILISK_COMPUTE_WIDTH]);
        int i;
        for (i = 0; i < BASILISK_COMPUTE_WIDTH; i++) begin
    ...
    ) next_mult_command[BASILISK_COMPUTE_WIDTH] (
        .clk,
        .rst
    );
    std_stream_intf #(
        .T(basilisk_add_command_t)
    ) next_add_command[BASILISK_COMPUTE_WIDTH] (
        .clk,
        .rst
    );
    std_stream_intf #(
        .T(basilisk_sqrt_command_t)
    ) next_sqrt_command[BASILISK_COMPUTE_WIDTH] (
        .clk,
        .rst
    );
    std_stream_intf #(
        .T(basilisk_divide_command_t)
    ) next_divide_command[BASILISK_COMPUTE_WIDTH] (
        .clk,
        .rst
    );
    ...
    generate
        for (k = 0; k < BASILISK_COMPUTE_WIDTH; k++) begin
            ...
            std_flow_stage #(
                ...
                .clk,
                .rst,
                .stream_in (next_mult_command[k]),
                .stream_out(mult_command[k])
            );
            ...
            std_flow_stage #(
                ...
                .clk,
                .rst,
                .stream_in (next_add_command[k]),
                .stream_out(add_command[k])
            );
            ...
            std_flow_stage #(
                ...
                .clk,
                .rst,
                .stream_in (next_sqrt_command[k]),
                .stream_out(sqrt_command[k])
            );
            ...
            std_flow_stage #(
                ...
                .clk,
                .rst,
                .stream_in (next_divide_command[k]),
                .stream_out(divide_command[k])
            );
        end
    endgenerate
    ...
    std_flow_lite #(
        ...
        .consume,
        .produce({
            produce_encode, produce_convert, produce_memory, produce_mult, produce_add, produce_sqrt, produce_divide
        }),
        .enable
    );
    ...
    std_flow_stage #(
        ...
        .clk,
        .rst,
        .stream_in (next_encode_command),
        .stream_out(encode_command)
    );
    ...
    std_flow_stage #(
        ...
        .clk,
        .rst,
        .stream_in (next_convert_command),
        .stream_out(convert_command)
    );
    ...
    std_flow_stage #(
        ...
        .clk,
        .rst,
        .stream_in (next_memory_command),
        .stream_out(memory_command)
    );
    ...
    always_comb begin
            ...
            RV32V_OPCODE_OP: begin
                case (rv32v_funct3_t'(inst_fields.funct3))
                    ...
                    RV32V_FUNCT3_OP_FVV: begin  // Floating Point Vector-Vector
                        case (rv32v_funct6_t'(inst_fields.funct6))
                            RV32V_FUNCT6_VFMACC, RV32V_FUNCT6_VFNMACC, RV32V_FUNCT6_VFMSAC, RV32V_FUNCT6_VFNMSAC:
                            enable_vector_macc = 'b1;
        ...
        selected_round_mode =
            basilisk_decode_find_rounding_mode(rv32f_funct3_round_t'(inst_fields.funct3), current_round_mode);
        ...
        case (float_command.payload.sys_op)
            RV32I_FUNCT3_SYS_CSRRWI, RV32I_FUNCT3_SYS_CSRRSI, RV32I_FUNCT3_SYS_CSRRCI:
            sys_value = float_command.payload.sys_imm;
            default: sys_value = float_command.payload.rs1_value;
        endcase

        incoming_round_mode =
            basilisk_decode_find_rounding_mode(rv32f_funct3_round_t'(sys_value[2:0]), FPU_ROUND_MODE_EVEN);
        ...
        if (current_state == BASILISK_DECODE_STATE_RESET) begin
            ...
            if (basilisk_decode_depend_registers(
                    inst_fields, rd_vector_status, rs1_vector_status, rs2_vector_status, rs3_vector_status
