...
#(
    ...
    std_stream_intf #(
        .T(basilisk_result_t)
    ) writeback_results_in[WRITEBACK_PORTS] (
        .clk,
        .rst
    );

    stream_tie stream_tie_inst0 (
        .stream_in (add_result_command),
        .stream_out(writeback_results_in[0])
    );
    stream_tie stream_tie_inst1 (
        .stream_in (mult_result_command),
        .stream_out(writeback_results_in[1])
    );
    stream_tie stream_tie_inst2 (
        .stream_in (divide_result_command),
        .stream_out(writeback_results_in[2])
    );
    stream_tie stream_tie_inst3 (
        .stream_in (sqrt_result_command),
        .stream_out(writeback_results_in[3])
    );

    generate
        if (ENABLE_MEMORY_CONVERT) begin
            stream_tie stream_tie_inst4 (
                .stream_in (memory_result),
                .stream_out(writeback_results_in[4])
            );
            stream_tie stream_tie_inst5 (
                .stream_in (convert_result),
                .stream_out(writeback_results_in[5])
            );
        end
    endgenerate
