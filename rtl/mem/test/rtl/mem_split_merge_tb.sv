...
module mem_split_merge_tb ();
    ...
    mem_merge #(.PORTS(2),.META_WIDTH(1),.USE_LAST(1)) merge_inst(
        .clk,.rst,.mem_in(mi),.mem_in_meta('{meta_i[0],meta_i[1]}),.mem_out(mm),.mem_out_meta(meta_m));
    mem_split #(.PORTS(2),.META_WIDTH(1),.USE_LAST(1)) split_inst(
        .clk,.rst,.mem_in(mm),.mem_in_meta(meta_m),.mem_out(mo),.mem_out_meta('{meta_o[0],meta_o[1]}));
endmodule
