...
#(
    ...
    typedef enum logic [3:0] {
        ...
        CHILD_EVICTION_REQUESTS,
        CHILD_EVICTION_RESPONSES,
    ...
    function automatic line_search_t search_line(input line_meta_t [ASSOCIATIVITY-1:0] meta, input tag_t tag);
        line_search_t search = '{default: 'b0};

        // Search associated lines for hit, empty, and last used lines
        for (int i = 0; i < ASSOCIATIVITY; i++) begin
            if (cache_is_mesi_valid(meta[i].mesi) && meta[i].tag == tag) begin
                search.found_hit = 'b1;
                search.line = i;
            end
    ...
    function automatic addr_t encode_address(input tag_t tag, input index_t index, input block_t block);
        return {tag, index, block, {WORD_BITS{1'b0}}};
    endfunction

    function automatic addr_t encode_sub_address(input tag_t tag, input index_t index, input sub_block_t sub_block);
        return {tag, index, sub_block, {(WORD_BITS + $clog2(DATA_WIDTH_RATIO)) {1'b0}}};
    endfunction

    function automatic local_addr_t get_local_address(input index_t index, input block_t block, input lru_t assoc);
        return (ASSOCIATIVITY > 1) ? {index, assoc, block} : {index, block};
    endfunction

    // Lower LRU means it was modified the latest
    function automatic line_meta_t [ASSOCIATIVITY-1:0] update_lru(input line_meta_t [ASSOCIATIVITY-1:0] meta_in,
    ...
    function automatic mask_t get_write_mask(input child_mask_t child_mask, input addr_t child_addr);
        return (DATA_WIDTH_RATIO > 1) ? child_mask <<
            (child_addr[$clog2(DATA_WIDTH_RATIO)+CHILD_WORD_BITS-1:CHILD_WORD_BITS] << $clog2(CHILD_DATA_WIDTH / 8)) :
            child_mask;
    endfunction
    ...
    mem_intf #(
        ...
        .ID_WIDTH  ($bits(child_request.id))
    ) next_local_request (
        .clk,
        .rst
    );
    ...
    stream_stage #(
        ...
        .clk,
        .rst,
        .stream_in (next_bypass_request),
        .stream_out(bypass_request)
    );
    ...
    mem_stage #(
        ...
        .clk,
        .rst,
        .mem_in (next_local_request),
        .mem_out(local_request)
    );
    ...
    stream_stage #(
        ...
        .clk,
        .rst,
        .stream_in (next_local_request_meta),
        .stream_out(local_request_meta)
    );
    ...
    std_register #(
        .CLOCK_INFO(CLOCK_INFO),
        .T(parent_request_t),
        .RESET_VECTOR('{op: CACHE_MESI_OPERATION_REJECT, default: 'b0})
    ) parent_request_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_parent_request),
        .value(current_parent_request)
    );
    std_register #(
        .CLOCK_INFO(CLOCK_INFO),
        .T(child_request_t),
        .RESET_VECTOR('{op: CACHE_MESI_OPERATION_REJECT, default: 'b0})
    ) child_request_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_child_request),
        .value(current_child_request)
    );
    ...
    std_register #(
        ...
        .T(addr_t),
        .RESET_VECTOR('b0)
    ) residual_address_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_residual_address),
        .value(current_residual_address)
    );
    ...
    std_register #(
        ...
        .T(logic [PORTS-1:0]),
        .RESET_VECTOR('b0)
    ) child_map_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_child_map),
        .value(current_child_map)
    );
    std_register #(
        .CLOCK_INFO(CLOCK_INFO),
        .T(child_id_t),
        .RESET_VECTOR('b0)
    ) child_id_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_child_id),
    ...
    std_register #(
        ...
        .T(logic),
        .RESET_VECTOR('b0)
    ) child_id_skip_enable_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_child_id_skip_enable),
        .value(current_child_id_skip_enable)
    );
    std_register #(
        .CLOCK_INFO(CLOCK_INFO),
        .T(child_id_t),
        .RESET_VECTOR('b0)
    ) child_id_skip_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_child_id_skip),
        .value(current_child_id_skip)
    );
    std_register #(
        .CLOCK_INFO(CLOCK_INFO),
        .T(logic),
        .RESET_VECTOR('b0)
    ) data_response_return_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_data_response_return),
        .value(current_data_response_return)
    );
    std_register #(
        .CLOCK_INFO(CLOCK_INFO),
        .T(logic),
        .RESET_VECTOR('b0)
    ) response_return_register_inst (
        .clk,
        .rst,
        .enable,
        .next (next_response_return),
        .value(current_response_return)
    );
    ...
    always_comb begin
        ...
        if (current_state == NORMAL) begin
            if (current_parent_request.valid) begin  // Process pending parent request
                ...
                consume_parent = 'b1;
                initiate_parent_request = 'b1;
                next_parent_request = '{valid: 'b1, op: parent_response_info.op, addr: parent_response.addr};
        ...
        case (current_state)
            ...
            BLOCK_WRITE: begin
                ...
                next_local_request.addr = get_local_address(decode_address_index(current_residual_address),
                                                            current_front_sub_block, current_residual_lru);
                next_local_request.last = (next_front_sub_block == 'b0);

                next_local_request_meta.payload = '{bypass_id: current_bypass_id, send_parent: 'b1, default: 'b0};
                next_local_request_meta.payload.addr = encode_sub_address(
                    decode_address_tag(
                        current_residual_address
                    ),
                    decode_address_index(
                        current_residual_address
                    ),
            ...
            BLOCK_READ: begin
                ...
                next_bypass_request.payload = '{bypass_id: current_bypass_id, send_parent: 'b1, default: 'b0};
                ...
                next_local_request.addr = get_local_address(decode_address_index(current_residual_address),
                                                            current_rear_sub_block, current_residual_lru);
            ...
            CHILD_SHARED_RESPONSE, CHILD_MODIFIED_RESPONSE: begin
                ...
                next_local_request.read_enable = 'b1;
                next_local_request.write_enable = 'b0;
                next_local_request.addr =
                    get_local_address(decode_address_index(current_child_request.addr), 'b0, current_residual_lru);
            ...
            CHILD_EVICTION_RESPONSES, CHILD_EVICTION_DATA_RESPONSE: begin
                ...
                case (child_request_info.op)
                    ...
                    CACHE_MESI_OPERATION_NORMAL_EVICT: begin
                        ...
                        if (search_results.line == current_residual_lru && decode_address_index(
                                child_request.addr
                            ) == decode_address_index(
                                current_residual_address
                            )) begin
                    ...
                    CACHE_MESI_OPERATION_NORMAL_EVICT_DATA: begin
                        ...
                        produce_local_request = 'b1;
                        next_local_request.read_enable = 'b0;
                        next_local_request.write_enable =
                            get_write_mask(child_request.write_enable, child_request.addr);
                        next_local_request.addr = get_local_address(decode_address_index(child_request.addr),
                                                                    current_front_sub_block, search_results.line);
            ...
            case (child_request_info.op)
                CACHE_MESI_OPERATION_SHARED: begin
                    ...
                    if (search_results.found_hit) begin
                        ...
                        if (metadata_read_data[search_results.line].modifiers) begin
                            ...
                            next_local_request.read_enable = 'b1;
                            next_local_request.write_enable = 'b0;
                            next_local_request.addr = get_local_address(decode_address_index(next_child_request.addr),
                                                                        'b0, search_results.line);
                            next_local_request.data = 'b0;
                ...
                CACHE_MESI_OPERATION_MODIFIED: begin
                    if (search_results.found_hit) begin
                        ...
                        end else if (HAS_COHERENT_PARENT && !cache_is_mesi_dirty(
                                metadata_read_data[search_results.line].mesi
                            )) begin
        ...
        if (!HAS_COHERENT_CHILD && initiate_child_request) begin
            ...
            if (search_results.found_hit) begin
                ...
                if (HAS_COHERENT_PARENT && next_child_request.write_enable && !cache_is_mesi_dirty(
                        metadata_read_data[search_results.line].mesi
                    )) begin
                    // Issue request for the data to be upgraded
                    produce_bypass_request = 'b1;
                    ...
                    next_local_request.write_enable =
                        get_write_mask(next_child_request.write_enable, next_child_request.addr);
                    next_local_request.addr = get_local_address(
                        decode_address_index(
                            next_child_request.addr
                        ),
                        decode_address_block(
                            next_child_request.addr
                        ),
                        search_results.line
                    );
                ...
                if (search_results.found_empty) begin
                    if (HAS_COHERENT_PARENT) begin
                        ...
                        next_bypass_request.payload = '{
                            ...
                            send_parent: 'b1,
                            op:
                            next_child_request.write_enable
                            ?
                            CACHE_MESI_OPERATION_MODIFIED
                            :
                            CACHE_MESI_OPERATION_SHARED,
                            addr: next_child_request.addr,
                            last: 'b1,
                            default: 'b0
                        };
        ...
        if (initiate_complete_eviction) begin
            ...
            if (IS_COHERENT && !IS_FIRST_LEVEL && metadata_read_data[next_residual_lru].modifiers) begin
                ...
                next_local_request.addr =
                    get_local_address(decode_address_index(next_residual_address), 'b0, next_residual_lru);
                next_local_request.last = 'b0;
                ...
                if (IS_COHERENT && !IS_LAST_LEVEL) begin
                    ...
                    next_local_request_meta.payload.addr = encode_sub_address(
                        decode_address_tag(next_residual_address), decode_address_index(next_residual_address), 'b0);
                end
