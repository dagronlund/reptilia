package cache_pkg;
    ...
    typedef struct packed {cache_mesi_operation_t op;} cache_mesi_request_t;

    typedef struct packed {cache_mesi_operation_t op;} cache_mesi_response_t;

    function automatic logic cache_is_mesi_valid(input cache_mesi_state_t state);
        return (state == CACHE_MESI_SHARED) || (state == CACHE_MESI_EXCLUSIVE) || (state == CACHE_MESI_MODIFIED);
    endfunction

    function automatic logic cache_is_mesi_dirty(input cache_mesi_state_t state);
